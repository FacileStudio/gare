...
var composeFileCandidates = []string{"compose.yml", "compose.yaml", "docker-compose.yml", "docker-compose.yaml"}
...
// DetectComposeFile returns the name of the first compose file found in repoDir.
...
// LocateComposeFile returns the compose file to run, defaulting to discovery in repoDir.
func LocateComposeFile(repoDir, configured string) (string, error) {
	...
	if detected := DetectComposeFile(repoDir); detected != "" {
		return detected, nil
	}
	return "", fmt.Errorf("no compose file found in repository (looked for %s)", strings.Join(composeFileCandidates, ", "))
}

func validateComposeFileName(name string) error {
...
// ComposePublishedPorts returns the host ports published by a compose file.
func ComposePublishedPorts(composePath string) ([]int, error) {
...
// ValidateComposePort ensures the configured port is published by the compose file.
func ValidateComposePort(composePath string, port int) error {
	if port <= 0 {
		return fmt.Errorf("compose workloads require an explicit port in gare configuration")
	}
	ports, err := ComposePublishedPorts(composePath)
	if err != nil {
		return err
	}
